...
class PrinciplesReport(ExpertReport):
    ...
    dinner_test_failures: list[dict] = Field(default_factory=list, description="Sentences that fail the dinner test")
...
class DesktopUXReport(ExpertReport):
    ...
    five_second_test: dict[str, str] = Field(default_factory=dict, description="Per-page 5-second test result")
...
class ConversionReport(ExpertReport):
    ...
    dead_ends: list[dict] = Field(default_factory=list, description="Pages with no clear next action")
    cta_issues: list[dict] = Field(default_factory=list, description="CTA problems: missing, weak, competing")
    cross_site_gaps: list[dict] = Field(default_factory=list, description="Missing cross-links to sails.to or related sites")
    funnel_leaks: list[dict] = Field(default_factory=list, description="Points where users likely drop out of the funnel")
