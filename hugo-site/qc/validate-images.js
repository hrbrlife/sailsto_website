...
function extractImageReferences(files) {
    ...
    const patterns = [
        { regex: /<img[^>]+src="([^"]+)"[^>]*>/gi, altRegex: /alt="([^"]*)"/i, type: 'img' },
    ...
    for (const file of files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            // Check img tags specifically for alt text
            ...
            for (const { regex, type } of patterns) {
                if (type === 'img') continue; // Already handled above
...
function validateImages() {
    ...
    const brokenImages = references.filter(ref => {
        // Only check images that look like static files
    ...
    if (brokenImages.length === 0) {
        ...
        for (const [url, refs] of byUrl) {
            ...
            if (refs.length > 2) {
                console.log(`    ${DIM}... and ${refs.length - 2} more${RESET}`);
            }
        }
        console.log();
    }
    ...
    if (missingAlt.length === 0) {
        ...
        if (missingAlt.length > 10) {
            console.log(`  ${DIM}... and ${missingAlt.length - 10} more${RESET}`);
        }
        console.log();
    }
    ...
    // Filter out likely dynamic/favicon images
    ...
    if (filteredUnused.length === 0) {
        ...
        for (const img of filteredUnused) {
            console.log(`  ${YELLOW}•${RESET} ${img}`);
        }
