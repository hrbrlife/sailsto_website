...
/**
 * Contrast & Visibility Validation Script
 * 
 ...
 * 
 * Requirements:
 *   npm install (from qc/ directory)
 *   npx playwright install chromium
 *   Hugo dev server running (http://localhost:1313)
 * 
...
const BASE_URL = process.argv.find(a => a.startsWith('--base-url='))
    ?.split('=')[1] || process.env.BASE_URL || 'http://localhost:1313';

const VERBOSE = process.argv.includes('--verbose') || process.argv.includes('-v');

// WCAG 2.1 AA thresholds
const WCAG_AA_NORMAL = 4.5;  // Normal text
const WCAG_AA_LARGE  = 3.0;  // Large text (≥24px or ≥18.66px bold)

// Skip invisible / intentionally hidden elements
const SKIP_SELECTORS = ['script', 'style', 'noscript', 'template', 'svg', 'path'];
...
/**
 * WCAG "large text": >= 24px, or >= 18.66px if bold (weight >= 700)
 */
...
/**
 * Recursively discover all content pages as URL paths
 */
 ...
 * It walks all visible text nodes and extracts color data.
 */
const BROWSER_EXTRACT_FN = function () {
    ...
    function getEffectiveBackground(el) {
        // Collect all ancestor backgrounds from element to root
        const backgrounds = [];
        let current = el;
        ...
        // Add html element background
        if (document.documentElement) {
            backgrounds.push(parseColor(getComputedStyle(document.documentElement).backgroundColor));
        }

        // Composite from root to element, starting with white (browser default)
...
async function validateContrast(options = {}) {
    ...
    if (!chromium) {
        ...
        console.log(`  Run: ${CYAN}cd qc && npm install && npx playwright install chromium${RESET}\n`);
    ...
    if (!serverUp) {
        ...
        console.log(`  Start the Hugo dev server: ${CYAN}hugo server${RESET}\n`);
        return { success: true, errors: 0, warnings: 1, skipped: true };
    }
    ...
    try {
        ...
        console.log(`  Run: ${CYAN}cd qc && npx playwright install chromium${RESET}\n`);
        return { success: true, errors: 0, warnings: 1, skipped: true };
    }
    ...
    let pagesChecked = 0;
    let pagesFailed = 0;
    const allViolations = [];  // for summary

    for (let i = 0; i < contentPages.length; i++) {
        const { url: pagePath, file } = contentPages[i];
        ...
        try {
            ...
            // Skip if page not found (draft, etc.)
            ...
            // Extract contrast data from the page
            ...
            for (const el of elements) {
                // Skip elements on background images (can't reliably determine contrast)
                if (el.hasBgImage) {
                    pageBgImageWarns++;
                    continue;
                }

                // Composite semi-transparent text with background
            ...
            if (pageViolations.length === 0) {
                process.stdout.write(` ${GREEN}✓${RESET}${pageBgImageWarns > 0 ? ` ${DIM}(${pageBgImageWarns} bg-image)${RESET}` : ''}\n`);
            } else {
                ...
                for (const v of pageViolations) {
                    const ratioStr = v.ratio.toFixed(1) + ':1';
                    const needStr = v.threshold + ':1';
                    const sizeInfo = v.largeText ? ' (large)' : '';
                    const textColor = colorToHex(v.effectiveText);
                    const bgColor = colorToHex(v.bgColor);

                    if (VERBOSE || pageViolations.length <= 8) {
                        console.log(`    ${RED}✗${RESET} [${ratioStr} < ${needStr}] ${DIM}${v.selector}${sizeInfo}${RESET}`);
                        console.log(`      text ${BOLD}${textColor}${RESET} on bg ${BOLD}${bgColor}${RESET} — "${v.text.substring(0, 50)}"`);
                    }
                }

                if (!VERBOSE && pageViolations.length > 8) {
                    console.log(`    ${DIM}... showing first 0, pass --verbose to see all ${pageViolations.length}${RESET}`);
                    // Still show the worst offenders
                    const worst = pageViolations.sort((a, b) => a.ratio - b.ratio).slice(0, 5);
                    for (const v of worst) {
                        const ratioStr = v.ratio.toFixed(1) + ':1';
                        const needStr = v.threshold + ':1';
                        const textColor = colorToHex(v.effectiveText);
                        const bgColor = colorToHex(v.bgColor);
                        console.log(`    ${RED}✗${RESET} [${ratioStr} < ${needStr}] ${DIM}${v.selector}${RESET}`);
                        console.log(`      text ${BOLD}${textColor}${RESET} on bg ${BOLD}${bgColor}${RESET} — "${v.text.substring(0, 50)}"`);
                    }
                    console.log(`    ${DIM}... and ${pageViolations.length - 5} more${RESET}`);
                }

                if (pageBgImageWarns > 0) {
                    console.log(`    ${YELLOW}⚠${RESET} ${DIM}${pageBgImageWarns} element(s) on background image (manual check needed)${RESET}`);
                }
            }
    ...
    if (allViolations.length > 0) {
        console.log(`${BOLD}  Worst pages:${RESET}`);
        const sorted = allViolations.sort((a, b) => b.violations.length - a.violations.length);
        for (const pg of sorted.slice(0, 10)) {
            const worstRatio = Math.min(...pg.violations.map(v => v.ratio));
            console.log(`    ${RED}${pg.violations.length.toString().padStart(3)}${RESET} violations  ${pg.path}  ${DIM}(worst: ${worstRatio.toFixed(1)}:1)${RESET}`);
        }
        console.log();
    }
    ...
    if (totalViolations === 0) {
        ...
        console.log(`${DIM}  WCAG AA requires ≥${WCAG_AA_NORMAL}:1 for normal text, ≥${WCAG_AA_LARGE}:1 for large text (≥24px or ≥18.66px bold)${RESET}\n`);
