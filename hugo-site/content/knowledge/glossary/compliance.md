---
title: "Compliance - Glossary"
description: "Adherence to legal, regulatory, and internal requirements — automated and enforced on-chain for CrossSecurities through KYC/AML, investor eligibility, and transfer restrictions."
ogImage: "/og-image.png"
keywords: ["compliance", "glossary", "adherence", "legal", "regulatory", "internal", "requirements", "automated"]
stylesheets:
  - "/assets/fonts/fonts.css"
  - "/styles.css"
  - "/assets/css/glossary.css"
  - "/assets/css/glossary-term.css"
---

<header class="term-page-header">
    <div class="container">
        <a href="/knowledge/glossary/" class="back-link">← Back to Glossary</a>
        <span class="term-category">Compliance</span>
        <h1 class="term-title">Compliance</h1>
        <p class="term-short">Adherence to legal, regulatory, and internal requirements — automated and enforced on-chain for CrossSecurities through KYC/AML, investor eligibility, and transfer restrictions.</p>
    </div>
</header>
<main class="term-content">
    <div class="term-content-inner">
        <section class="term-section">
            <h2>Full Definition</h2>
            <p>Securities compliance encompasses all rules governing how securities can be issued, traded, and held. On Sails.to, compliance is built into the infrastructure:</p>
            <ul>
                <li><strong>KYC/AML</strong> — Identity verification and anti-money laundering checks before any transaction</li>
                <li><strong>Investor eligibility</strong> — On-chain verification that buyers meet accredited/professional status</li>
                <li><strong>Transfer restrictions</strong> — Smart contracts enforce holding periods and geographic restrictions</li>
                <li><strong>Regulatory reporting</strong> — Automated generation of required regulatory filings</li>
            </ul>
        </section>
        <section class="term-section">
            <h2>Why It Matters</h2>
            <p>Compliance is non-negotiable in securities. Violations can result in fines, criminal liability, and voided transactions. By automating compliance in smart contracts, we eliminate human error and ensure every transaction is valid by construction.</p>
            <p>This "compliant by design" approach lets issuers focus on their business while we handle the regulatory complexity.</p>
        </section>
        <section class="term-section">
            <h2>Related Terms</h2>
            <div class="related-terms">
                <a href="/knowledge/glossary/kyc/" class="related-term-link">KYC</a>
                <a href="/knowledge/glossary/aml/" class="related-term-link">AML</a>
                <a href="/knowledge/glossary/reg-d/" class="related-term-link">Regulation D</a>
                <a href="/knowledge/glossary/reg-s/" class="related-term-link">Regulation S</a>
                <a href="/knowledge/glossary/smart-contract/" class="related-term-link">Smart Contract</a>
            </div>
        </section>
        <div class="term-cta">
            <h3>Compliance built in</h3>
            <p>Focus on your raise, not regulatory paperwork.</p>
            <a href="/compliance/" class="btn">Learn About Compliance</a>
        </div>
    </div>
</main>
<script src="/assets/js/glossary.js"></script>
